...
class Modeltheme_Demo_Importer {
	...
	public function __construct() {
		...
		if ( get_transient( 'mt_importer_webp' ) ) {
	...
	public static function save( $key, $value ) {
		$demo = get_option( 'mt-downloaded-demo' );
		$option = (array) get_option( 'mt_uninstall_' . $demo );
		if ( $key && $value ) {
			$option[ $key ][] = $value;
			update_option( 'mt_uninstall_' . $demo, $option );
		}
	}
	...
	public static function replace_demo_link( $data = '', $encoded = false, $only_urls = false, $custom_folder = '' ) {
		$demo = get_option( 'mt-downloaded-demo' );
		$folder = get_option( 'mt-downloaded-folder' );
		...
		if ( strpos( $data, 'http' ) === 0 ) {
			if ( ! Modeltheme_Sites::contains( $data, [ '.jpg', '.webp', '.png', '.gif', '.jpeg', '.mp4', '.mp3' ] ) ) {
				$data = trailingslashit( $data );
			}
		}

		$api = apply_filters( 'mt_config_api', apply_filters( 'mt_config_api_demos', Modeltheme_Sites::$api ) );
	...
	public static function import_process( $args = false ) {
		...
		foreach( $args['features'] as $i => $key ) {
			...
			if ( $key === 'images' ) {
			    ...
			    $request = $path.'options.json';
				$json = file_get_contents($request);
	    		$json_array = maybe_unserialize($json);

			    $theme_option = 'theme_mods_'.MODELTEMA_THEME_NAME;
			    update_option($theme_option, $json_array);

				do_action( 'modeltheme/importer/after_import_theme_options', $options );
				...
				if ( $widgets ) {
					...
					do_action( 'modeltheme/importer/after_import_widgets' );
				...
				$mt_menus = json_decode(file_get_contents($menus_path), true);
				set_theme_mod( 'nav_menu_locations', $mt_menus['menus'] );



				// Set home page
				$homes_path = $path.'homes.json'; 
				$mt_homes = json_decode(file_get_contents($homes_path), true);
				$homepage = '';
				foreach($mt_homes['homepages'] as $home){
					$homepage = $home['get_page'];
				}
				...
				do_action( 'modeltheme/importer/after_import_content' );
			...
			// Import Sliders
				...
				do_action( 'modeltheme/importer/after_import_slider' );
				return [
					'status' 	=> '200',
	...
	public static function import_content( $file, $posts = 0 ) {
		...
		if ( ! class_exists( 'Modeltheme_WP_Import' ) ) {
		...
		if ( $importer_error ) {
			...
			if( ! is_file( $file ) ) {
				...
				$wp_import = new Modeltheme_WP_Import();
	...
	public static function import_revslider( $demo, $path = '' ) {
		...
		add_action( 'add_attachment', [ 'Modeltheme_Demo_Importer', 'revslider_add_attachment' ] );
		...
		// Replace links.
		$urls = Modeltheme_Demo_Importer::replace_demo_link( false, true, true );
	...
	public static function import_widgets( $data ) {
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = isset( $widget->title ) ? $widget->title : esc_html__( 'No Title', 'modeltheme' );
	...
	public static function download( $demo, $folder = '', $try = 1 ) {
		...
		if ( ! get_transient( 'mt_importer_webp' ) ) {
			set_transient( 'mt_importer_webp', true, 3600 );
		}

		// Save requested demo name.
		update_option( 'mt-downloaded-demo', $demo );
		update_option( 'mt-downloaded-folder', $folder );
		$api = apply_filters( 'mt_config_api', apply_filters( 'mt_config_api_demos', Modeltheme_Sites::$api ) );


		// Download URL.
		$remote = MODELTHEME_DEMO_DATA_FOLDER.$demo.'.zip';
		// Upload directory
		$dir = '/demo_data/';
		$up = wp_upload_dir();
		...
		if ( ! is_writable( $dir ) ) {
			...
			return [
				'status' 	=> '202',
				'message' 	=> esc_html__( 'Your uploads folder is not writable, please change it\'s permission to 0777', 'modeltheme' )
		...
		// Check directory
		if ( Modeltheme_Sites::contains( $dir, 'demo_data' ) ) {
			...
			if ( ! self::file_length( $zip ) ) {
				...
				if ( function_exists( 'curl_init' ) ) {
					...
					return [
						'status' 	=> '202',
						'message' 	=> esc_html__( 'Your server PHP cURL is not enable, Please contact with your hosting support.', 'modeltheme' )
					];
				}
			...
			if ( ! self::file_length( $zip ) ) {
				return [
					'status' 	=> '202',
					'message' 	=> esc_html__( 'Download failed, Please makes sure your theme and plugins are up to date and If still get the same error, your server blocked PHP for downloading ZIP files, Please contact with your server support.', 'modeltheme' )
				];
			}
			...
			if ( file_exists( $zip ) ) {
				unzip_file( $zip, $dir );
				unlink( $zip );
				update_option( 'mt_demo_path', $dir . $demo . '/' );
				return [
					'status' 	=> '200',
					'message' 	=> esc_html__( 'Demo file downloaded successfully.', 'modeltheme' )
				];
			} else {
				if ( $try ) {
					...
					return [
						'status' 	=> '202',
						'message' 	=> esc_html__( 'Something went wrong, Please contact with theme support.', 'modeltheme' )
					];
				}
			}
		...
		return [
			'status' 	=> '202',
			'message' 	=> esc_html__( 'Download demo failed, importer could not create demo folder.', 'modeltheme' )
		];
	}
	...
	public function process_attachment( $post, $url ) {
		...
		if ( is_wp_error( $pre_process ) ) {
			return array(
				...
				'text' => sprintf( esc_html__( '%1$s was not uploaded. (%2$s: %3$s)', 'modeltheme' ), $post['post_title'], $pre_process->get_error_code(), $pre_process->get_error_message() )
			);
		}
		...
		if ( is_wp_error( $upload ) ) {
			return array(
				...
				'text' => sprintf( esc_html__( '%1$s could not be uploaded because of an error. (%2$s: %3$s)', 'modeltheme' ), $post['post_title'], $upload->get_error_code(), $upload->get_error_message() )
			);
		}
		...
		if ( $info = wp_check_filetype( $upload['file'] ) ) {
			...
			$upload = new WP_Error( 'attachment_processing_error', esc_html__('Invalid file type', 'modeltheme') );
			return array(
				'fatal' => false,
				'type' => 'error',
				'code' => $upload->get_error_code(),
		...
		if ( Modeltheme_Sites::contains( $post['post_title'], 'woocommerce-placeholder' ) ) {
		...
		return array(
			'fatal' => false,
			'type' => 'updated',
			'text' => sprintf( esc_html__( '%s was uploaded successfully', 'modeltheme' ), $post['post_title'] )
		);
	}
		...
		if ( $imported ) {
			...
			foreach( $imported as $attachment ) {
				...
				if ( $post[ 'post_date_gmt' ] == $attachment->post_date_gmt ) {
					...
					return new WP_Error( 'duplicate_file_notice', esc_html__( 'File already exists', 'modeltheme' ) . ' ' . esc_html( $post[ 'post_title' ] ) );
	...
	public function fetch_remote_file( $url, $post ) {
		...
		if ( ! $tmp_file_name ) {
			return new WP_Error( 'import_no_file', esc_html__( 'Could not create temporary file.', 'modeltheme' ) );
		}
		...
		if ( is_wp_error( $remote_response ) ) {
			...
			return new WP_Error(
				...
				sprintf(
					...
					esc_html__( 'Request failed due to an error: %1$s (%2$s)', 'modeltheme' ),
		...
		if ( 200 !== $remote_response_code ) {
			...
			return new WP_Error(
				...
				sprintf(
					...
					esc_html__( 'Remote server returned the following unexpected result: %1$s (%2$s)', 'modeltheme' ),
		...
		if ( ! $headers ) {
			@unlink( $tmp_file_name );
			return new WP_Error( 'import_file_error', esc_html__('Remote server did not respond', 'modeltheme') );
		}
		...
		if ( 0 === $filesize ) {
			@unlink( $tmp_file_name );
			return new WP_Error( 'import_file_error', esc_html__('Zero size file downloaded', 'modeltheme') );
		}

		if ( ! isset( $headers['content-encoding'] ) && isset( $headers['content-length'] ) && $filesize !== (int) $headers['content-length'] ) {
			@unlink( $tmp_file_name );
			return new WP_Error( 'import_file_error', esc_html__('Downloaded file has incorrect size', 'modeltheme' ) );
		}

		$max_size = (int) apply_filters( 'import_attachment_size_limit', 0 );
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			@unlink( $tmp_file_name );
			return new WP_Error( 'import_file_error', sprintf(esc_html__('Remote file is too large, limit is %s', 'modeltheme' ), size_format($max_size) ) );
		}
		...
		if ( ( ! $type || ! $ext ) && ! current_user_can( 'unfiltered_upload' ) ) {
			return new WP_Error( 'import_file_error', esc_html__( 'Sorry, this file type is not permitted for security reasons.', 'modeltheme' ) );
		}
		...
		if ( ! $move_new_file ) {
			@unlink( $tmp_file_name );
			return new WP_Error( 'import_file_error', esc_html__( 'The uploaded file could not be moved', 'modeltheme' ) );
		}
