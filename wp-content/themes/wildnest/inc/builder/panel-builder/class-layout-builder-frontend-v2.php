 ...
 * @since 0.2.9
 */
class Wildnest_Layout_Builder_Frontend_V2  extends Wildnest_Abstract_Layout_Frontend {
	...
	public function render_item( $item_id, $col_id, $row_id, $device ) {
		 ...
		 * @since 0.2.1
		 */
		do_action( 'wildnest/builder/' . $this->id . '/before-item/' . $item_id );
		$object_item = Wildnest_Customize_Layout_Builder()->get_builder_item( $this->id, $item_id );
		 ...
		 * @since 0.2.1
		 */
		do_action( 'wildnest/builder/' . $this->id . '/after-item/' . $item_id );
				...
				foreach ( $col_items as $item_index => $col_item ) {
					...
					if ( $item ) {
						...
						if ( $content ) {
							...
							echo '<div class="' . esc_attr( $item_classes ) . '" data-section="' . esc_attr($item_config['section']) . '" data-item-id="' . esc_attr( $item_id ) . '" >'.$this->setup_item_content( $content, $id, $device );
							if ( is_customize_preview() ) {
			...
			if ( $show && isset( $this->rows[ $row_id ] ) ) {
				...
				if ( ! empty( $show_on_devices ) ) {
					...
					if ( ! empty( $desktop_row ) || ! empty( $mobile_row ) ) {
						...
						$align_classes = 'wildnest-grid-middle';
						...
						$show_on_sticky    = Wildnest()->get_setting( $this->id . '_' . $row_id . '_sticky_status' );
						if (!$show_on_sticky) {
							$classes[] = 'wildnest-hide-on-sticky';
						}
								...
								<div class="<?php echo join( ' ', $inner_class ); ?>">
									<div class="container">
										...
										if ( $html_desktop_escaped ) {
											...
											if ( $desktop_row ) {
												echo ''.$this->render_row( $desktop_row, $row_id, 'desktop' ).'';
											} else {
												false;
											}
											echo '</div>';
										...
										if ( $html_mobile_escaped ) {
											...
											if ( $mobile_row ) {
												echo ''.$this->render_row( $mobile_row, $row_id, 'mobile' ).'';
											} else {
												false;
											}
											echo '</div>';
		...
		if ( ! empty( $mobile_items ) || is_customize_preview() ) {
			...
			foreach ( $mobile_items as $row_id => $col_items ) {
				foreach ( $col_items as $item_index => $item ) {
					...
					echo '<div class="' . esc_attr( $inner_classes ) . '" data-item-id="' . esc_attr( $item_id ) . '" data-section="' . esc_attr($item_config['section']) . '">'.$this->setup_item_content( $content_escaped, $id, 'mobile' );
					if ( is_customize_preview() ) {
						echo '<span class="item--preview-name">' . esc_html( $item_config['name'] ) . '</span>';
					}
					echo '</div>';
...
/**
 * Alias of class Wildnest_Layout_Builder_Frontend_V2
 *
 * @see Wildnest_Layout_Builder_Frontend
 *
 * @return Wildnest_Layout_Builder_Frontend_V2
 */
function Wildnest_Layout_Builder_Frontend_V2() {
	return Wildnest_Layout_Builder_Frontend_V2::get_instance();
}
