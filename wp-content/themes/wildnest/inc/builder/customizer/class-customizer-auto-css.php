...
class Wildnest_Customizer_Auto_CSS {
	 ...
	 * Get intance.
	 *
	 * @return Wildnest_Customizer_Auto_CSS
	 */
	...
	private function replace_value($value, $format, $value_no_unit = null) {
	    // Check if $value_no_unit is null, replace with an empty string if so
	    $value_no_unit = $value_no_unit ?? '';

	    // Replace placeholders in $format
	    $s = str_replace('{{value}}', $value, $format);
	    $s = str_replace('{{value_no_unit}}', $value_no_unit, $s);
	...
	function setup_shadow( $value, $format ) {
		...
		$value    = Wildnest_Sanitize_Input::sanitize_color( $p['color'] );
		$color    = $value;
	...
	function setup_color( $value, $format ) {
		$value = Wildnest_Sanitize_Input::sanitize_color( $value );
		if ( $format ) {
	...
	function setup_image( $value, $format ) {
		$image = Wildnest()->get_media( $value );
		if ( $format ) {
			if ( $image ) {
	...
	function styling( $field, $values = null ) {
		$values = Wildnest()->get_setting( $field['name'], 'all' );
		...
		if ( isset( $values['normal']['bg_image'] ) ) {
			$image = Wildnest()->get_media( $values['normal']['bg_image'] );
			if ( ! $image ) {
		...
		foreach ( Wildnest()->customizer->devices as $device ) {
			$css = '';
			if ( isset( $selectorCSSDevices[ $device ] ) ) {
	...
	function maybe_devices_setup( $field, $call_back, $values = null, $no_selector = false ) {
		...
		if ( isset( $field['device_settings'] ) && $field['device_settings'] ) {
			...
			foreach ( Wildnest()->customizer->devices as $device ) {
				$value = null;
				if ( is_null( $values ) ) {
					$value = Wildnest()->get_setting( $field['name'], $device );
				} else {
			...
			if ( is_null( $values ) ) {
				$values = Wildnest()->get_setting( $field['name'] );
			}
			if ( method_exists( $this, $call_back ) ) {
		...
		$code_array = apply_filters( 'wildnest/customizer/auto_css', $code_array, $field, $this );
	...
	function typography( $field ) {
		...
		$fields      = array();
		$devices_css = array();
		foreach ( Wildnest()->customizer->get_typo_fields() as $f ) {
			$fields[ $f['name'] ] = $f;
		}
		...
		$devices_css = apply_filters( 'wildnest/customizer/auto_css', $devices_css, $field, $this );
	...
	public function render_css( $fields = array() ) {
		...
		$css_code = apply_filters( 'wildnest/auto-css', $css_code, $this );
		return $css_code;
	}
	...
	function auto_css( $partial = false ) {
		...
		$config_fields = Wildnest()->customizer->get_config();
		/**
