...
(function($) {
	...
	var is_rtl = Wildnest_Layout_Builder.is_rtl;
	...
	CustomizeBuilder_V2 = function(options, id) {
		var Builder = {
			...
			drag_drop: function() {
				...
				$(".wildnest--device-panel", that.container).each(function() {
					var panel = $(this);
					...
					$(".col-items, .wildnest-available-items", panel).each(function() {
			...
			addPanel: function(device) {
				...
				var template_id = "tmpl-wildnest--cb-panel-v2";
				...
				return (
					'<div class="wildnest--device-panel wildnest-vertical-panel wildnest--panel-' +
					device +
					'" data-device="' +
					device +
					'">' +
			...
			addDevicePanels: function() {
				...
				_.each(that.devices, function(device_name, device) {
					...
					$(".wildnest--cb-devices-switcher", that.container).append(
					...
					$(".wildnest--cb-body", that.container).append(panelHTML);
				});

				if ($("#wildnest-upsell-tmpl").length) {
					$($("#wildnest-upsell-tmpl").html()).insertAfter(
						$(".wildnest--cb-devices-switcher", that.container)
					);
				}
			},
			addItem: function(node) {
				var that = this;
				var template = that.getTemplate();
				var template_id = "tmpl-wildnest--cb-item";
				if ($("#" + template_id).length == 0) {
					return;
				}
			...
			addAvailableItems: function() {
				...
				_.each(that.devices, function(device_name, device) {
					var $itemWrapper = $(
						'<div class="wildnest-available-items" data-device="' + device + '"></div>'
					);

					$(".wildnest--panel-" + device, that.container).append(
						$itemWrapper
					);
			...
			switchToDevice: function(device, toggle_button) {
				...
				if (numberDevices > 1) {
					$(
						".wildnest--cb-devices-switcher a",
						that.container
					).removeClass("wildnest--tab-active");
					$(
						".wildnest--cb-devices-switcher .switch-to-" + device,
						that.container
					).addClass("wildnest--tab-active");
					$(".wildnest--device-panel", that.container).addClass(
						"wildnest--panel-hide"
					);
					$(
						".wildnest--device-panel.wildnest--panel-" + device,
						that.container
					).removeClass("wildnest--panel-hide");
			...
			addNewWidget: function( device, row_id, col_id, node, index) {
				...
				row = $( '.wildnest--cb-row.wildnest--row-'+row_id, panel );
				col = $( '.col-items.col-'+col_id, row );

				var $item = $( '.wildnest-available-items .grid-stack-item[data-id="'+node.id+'"]', panel );
			...
			focus: function() {
				this.container.on(
					"click",
					".wildnest--cb-item-setting, .wildnest--cb-item-name, .item-tooltip",
					function(e) {
						e.preventDefault();
			...
			remove: function() {
				...
				$document.on(
					"click",
					".wildnest--device-panel .wildnest--cb-item-remove",
					function(e) {
						e.preventDefault();
						var item = $(this).closest(".grid-stack-item");
						var panel = item.closest(".wildnest--device-panel");
						item.removeAttr("style");
						$(".wildnest-available-items", panel).append(item);
						that.save();
					}
				);
			},
			...
			save: function() {
				...
				_.each( that.devices, function( device_label, device ){
					data[ device ] = {};
					var devicePanel = $( '.wildnest--panel-'+device, that.container );
					$( '.wildnest--cb-row', devicePanel ).each( function(){
						var row = $( this );
			...
			showPanel: function() {
				var that = this;
				this.container
					.removeClass("wildnest--builder--hide")
					.addClass("wildnest--builder-show");
				setTimeout(function() {
			...
			hidePanel: function() {
				this.container.removeClass("wildnest--builder-show");
				$("#customize-preview")
			...
			togglePanel: function() {
				...
				wpcustomize.state("expandedPanel").bind(function(paneVisible) {
					if (wpcustomize.panel(options.panel).expanded()) {
						$document.trigger("wildnest_panel_builder_open", [
							options.panel
						]);
				...
				that.container.on("click", ".wildnest--panel-close", function(
					e
				) {
					e.preventDefault();
					that.container.toggleClass("wildnest--builder--hide");
					if (that.container.hasClass("wildnest--builder--hide")) {
			...
			init: function(controlId, items, devices) {
				...
				var template = that.getTemplate();
				var template_id = "tmpl-wildnest--builder-panel";
				var html = template(options, template_id);
				that.container = $(html);
				that.container.addClass( 'wildnest--panel-v2' );
				...
				if (options.section) {
					wpcustomize
						...
						.container.addClass("wildnest--hide");
				}
				...
				that.container.on(
					"click",
					".wildnest--cb-devices-switcher a.switch-to",
					function(e) {
						e.preventDefault();
				...
				$document.trigger("wildnest_builder_panel_loaded", [id, that]);
