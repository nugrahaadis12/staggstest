(function($, wpcustomize) {
	...
	wpcustomize.bind("ready", function(e, b) {
		var addVersionChange = function(opts, builder, id, version) {
			if ( Wildnest_Layout_Builder.hide_switcher === 'yes' ) {
				return ;
			}
			...
			if (typeof opts.versions === "object") {
				...
				if (select_options) {
					var $select_options = $(
						'<span class="wildnest-swicth-version button-primary"><span>'+Wildnest_Layout_Builder.swicth_version+'</span><select class="builder-version-change ">' +
							select_options +
							"</select></span>"
					);
					$(".wildnest--cb-actions", builder.container).prepend(
						$select_options
					);
				}
				...
				$select_options.on("change", 'select', function() {
					var newVer = $(this).val();
					if ( wildnestPanels[id + version] ) {
						wildnestPanels[id + version].container.hide();
					}
					
					if (typeof wildnestPanels[id + newVer] === "undefined") {
						var _builder;
						if ("v2" === newVer) {
						...
						wildnestPanels[id + newVer] = _builder;
					}
					...
					if ( wildnestPanels[id + newVer] ) {
						wildnestPanels[id + newVer].container.show();
					}
		...
		_.each(Wildnest_Layout_Builder.builders, function(opts, id) {
			var builder;
			var version = "v1";
			...
			wildnestPanels[id + version] = builder;
				...
				if (
					...
					$(
						...
						'<p class="footer_moved_widgets_text">' +
							Wildnest_Layout_Builder.footer_moved_widgets_text +
							"</p>"
					);
				}
			}, 1000);
		...
		wpcustomize.state("expandedSection").bind(function(section) {
			$(".wildnest--device-panel .grid-stack-item").removeClass(
				"item-active"
			);
			$(".wildnest--cb-row").removeClass("row-active");
			if (section) {
				$('.wildnest--cb-row[data-id="' + section.id + '"]').addClass(
					"row-active"
				);
				$(
					".wildnest--device-panel .grid-stack-item.for-s-" +
						section.id
		...
		function(e) {
			...
			if (template_name && template_name !== "") {
				...
				$.post(
					ajaxurl,
					{
						action: "wildnest_builder_save_template",
						name: input.val(),
	...
	$document.on("mouseover", ".wildnest--cb-row .grid-stack-item:not(.no-tooltip)", function(
		e
	) {
		var item = $(this);
		var nameW =
			$(".wildnest--cb-item-remove", item).outerWidth() +
			$(".wildnest--cb-item-setting", item).outerWidth();
	...
	$document.on("mouseleave", ".wildnest--cb-row .grid-stack-item:not(.no-tooltip)", function(
		e
	) {
