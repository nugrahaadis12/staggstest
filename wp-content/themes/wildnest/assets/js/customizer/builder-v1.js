		...
		var Builder = {
			...
			drag_drop: function() {
				...
				$(".wildnest--device-panel", that.container).each(function() {
					...
					var sortable_ids = [];
					that.panels[device] = {};
					$(".wildnest--cb-items", panel).each(function(index) {
					...
					$(
						".wildnest-available-items .grid-stack-item",
						panel
					).draggable({
						revert: "invalid",
						...
						start: function(event, ui) {
							...
							$(".wildnest--cb-items", panel).css("z-index", "");
			...
			addNewWidget: function($item, row) {
				...
				var panel = that.container.find(
					".wildnest--device-panel.wildnest--panel-" +
						that.activePanel
				);
				var el = row;
				if (!_.isObject(el)) {
					el = panel.find(".wildnest--cb-items").first();
				}
			...
			addPanel: function(device) {
				...
				var template_id = "tmpl-wildnest--cb-panel";
			...
			addAvailableItems: function() {
				...
				_.each(that.devices, function(device_name, device) {
					var $itemWrapper = $(
						'<div class="wildnest-available-items" data-device="' +
							device +
							'"></div>'
					);
					...
					_.each(device_data, function(items, row_id) {
						if (!_.isUndefined(items)) {
							_.each(items, function(node, index) {
								var item = $(
									'.wildnest-available-items[data-device="' +
										device +
